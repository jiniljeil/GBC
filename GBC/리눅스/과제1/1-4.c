...
int main(void)
{
	...
	while(1)
	{
		printf("myshell %s>",getcwd(curDir,sizeof(curDir)));
		fgets(command,256,stdin);
		argCount = parseCmd(command,argList);
		if(strcmp(argList[0],"exit")==0)//사용자가 exit를 입력하면 종료됨 
		{
			break;
		}else if(strcmp(argList[0],"cd")==0)//사용자가 cd명령어를 쓰려고 할때
		{
			chdir(argList[1]);
		}else//그 이외의 명령어를 입력하면 그 명령어를 실행
		{
			if(strcmp(argList[argCount-1],"&")==0)//만약 명령어의 끝에 &를 붙였다면
			{
